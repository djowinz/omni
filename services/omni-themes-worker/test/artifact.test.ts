...
async function buildSignedBundle(opts: {
  ...
  const bytes = identity.packSignedBundle(manifest, filesMap, FIXTURE_SEED, undefined) as Uint8Array;
...
describe("PATCH /v1/artifact/:id — end-to-end DO roundtrip", () => {
  ...
  it("returns {status:'updated'} with new content_hash for a modified bundle", async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { status: string; content_hash: string; artifact_id: string };
    ...
    const row = await env.META.prepare(
      "SELECT content_hash, version FROM artifacts WHERE id = ?",
    ).bind("artUpd").first<{ content_hash: string; version: string }>();
