...
function deriveDeviceFingerprint(pubkey) {
  return new Uint8Array(createHash('sha256').update(pubkey).update('omni-test-df').digest());
}
...
async function main() {
  ...
  // ---- fixture 1: theme-only ----
  {
    ...
    const bytes = Identity.packSignedBundle(manifest, filesMap, TEST_KEYPAIR_SEED, undefined);
  ...
  // JWS verification.
  {
    ...
    if (cdOffset < 64) {
      throw new Error(`tamper: could not locate central directory (cdOffset=${cdOffset})`);
    }
  ...
  writeFileSync(resolve(outDir, 'fixtures.json'), JSON.stringify(index, null, 2) + '\n');
