...
describe("POST /v1/admin/pubkey/ban", () => {
  it("flags is_denied, writes KV mirror, cascades tombstones to all live artifacts", async () => {
    ...
    const res = await app.fetch(
      mkReq("POST", "/v1/admin/pubkey/ban", bodyBytes, jws),
      env,
    );
    ...
    const authorRow = await env.META.prepare(
      "SELECT is_denied FROM authors WHERE pubkey = ?",
    )
    ...
    const tombCount = await env.META.prepare(
      "SELECT COUNT(*) AS n FROM tombstones",
    ).first<{ n: number }>();
  ...
  it("idempotent: rerun returns cascade_count=0, no duplicate tombstone rows", async () => {
    ...
    const r1 = await app.fetch(
      mkReq("POST", "/v1/admin/pubkey/ban", bodyBytes, s1.jws),
      env,
    );
    ...
    const r2 = await app.fetch(
      mkReq("POST", "/v1/admin/pubkey/ban", bodyBytes, s2.jws),
      env,
    );
  ...
  it("invalid hex → Malformed.BadRequest", async () => {
    const app = mkApp();
    const bodyBytes = new TextEncoder().encode(
      JSON.stringify({ pubkey: "not-hex", reason: "x" }),
    );
    const { jws } = await signJws({
      method: "POST",
      path: "/v1/admin/pubkey/ban",
  ...
  it("missing reason → Malformed.BadRequest", async () => {
    const app = mkApp();
    const bodyBytes = new TextEncoder().encode(
      JSON.stringify({ pubkey: BANNED_AUTHOR_HEX }),
    );
    const { jws } = await signJws({
      method: "POST",
      path: "/v1/admin/pubkey/ban",
...
describe("POST /v1/admin/pubkey/unban", () => {
  it("clears is_denied + KV mirror; tombstones NOT resurrected", async () => {
    ...
    const banRes = await app.fetch(
      mkReq("POST", "/v1/admin/pubkey/ban", banBody, banSig.jws),
      env,
    );
    expect(banRes.status).toBe(200);

    // Now unban.
    const unbanBody = new TextEncoder().encode(
      JSON.stringify({ pubkey: BANNED_AUTHOR_HEX }),
    );
  ...
  it("non-moderator → Admin.NotModerator 403", async () => {
    ...
    const res = await app.fetch(
      mkReq("POST", "/v1/admin/pubkey/unban", bodyBytes, jws),
      env,
    );
    expect(res.status).toBe(403);
    const j = (await res.json()) as { kind: string; detail: string };
...
describe("POST /v1/admin/device/ban", () => {
  it("writes KV denylist entry with reason + at", async () => {
    ...
    const res = await app.fetch(
      mkReq("POST", "/v1/admin/device/ban", bodyBytes, jws),
      env,
    );
    expect(res.status, await res.clone().text()).toBe(200);
  ...
  it("invalid hex → Malformed.BadRequest", async () => {
    const app = mkApp();
    const bodyBytes = new TextEncoder().encode(
      JSON.stringify({ device_fp: "short", reason: "x" }),
    );
    const { jws } = await signJws({
      method: "POST",
      path: "/v1/admin/device/ban",
...
describe("POST /v1/admin/device/unban", () => {
  it("deletes the KV denylist entry", async () => {
    ...
    const app = mkApp();
    const bodyBytes = new TextEncoder().encode(
      JSON.stringify({ device_fp: TARGET_DEVICE_HEX }),
    );
    const { jws } = await signJws({
      method: "POST",
    ...
    const res = await app.fetch(
      mkReq("POST", "/v1/admin/device/unban", bodyBytes, jws),
      env,
    );
