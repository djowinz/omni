...
export function StatusBar() {
  ...
  const getOverlayType = (): { label: string; color: string } => {
    ...
    if (currentOverlay?.name === state.config?.active_overlay)
      return { label: 'Active', color: '#00D9FF' };
  ...
  return (
    <footer className="flex h-7 items-center justify-between border-t border-[#27272A] bg-[#18181B] px-4 text-[10px]">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Version */}
        <span className="text-[#52525B] font-mono">VERSION {process.env.OMNI_VERSION}</span>
      </div>
    </footer>
  );
}
