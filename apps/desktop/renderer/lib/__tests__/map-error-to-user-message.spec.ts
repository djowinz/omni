...
describe('mapErrorToUserMessage', () => {
  ...
  describe('text invariant', () => {
    it.each(ALL_KINDS)('text === error.message for kind=%s', (kind) => {
  ...
  describe('opaquePayload round-trip', () => {
    it.each(ALL_KINDS)('round-trips { code, kind, detail } for kind=%s', (kind) => {
      const error = makeError(kind);
      const mapped = mapErrorToUserMessage(error);
    ...
    it('preserves code verbatim', () => {
      ...
      const parsed = JSON.parse(mapErrorToUserMessage(error).opaquePayload);
  ...
  describe('missing detail handling', () => {
    ...
    it.each(ALL_KINDS)(
      ...
      (kind) => {
        ...
        expect(Object.prototype.hasOwnProperty.call(parsed, 'detail')).toBe(true);
      },
    );
  });
});
