...
**params**
```json
{ "kind": "theme|bundle|all", "sort": "new|installs|name",
  "tags": ["dark"], "cursor": null, "limit": 25 }
```
**result**
```json
{ "items": [ /* Worker list item */ ], "next_cursor": "…|null" }
```
...
**params**
```json
{ "artifact_id": "…", "target_workspace": "string",
  "overwrite": false, "expected_pubkey_hex": "<64-hex Ed25519 pubkey>|null" }
```
...
**result**
```json
{ "installed_path": "string", "content_hash": "…",
  "author_fingerprint_hex": "…", "tofu": "first_install|matched|mismatch",
  "warnings": ["…"] }
```
...
**result**
```json
{ "content_hash": "…", "compressed_size": 0, "uncompressed_size": 0,
  "manifest": { /* manifest */ }, "sanitize_report": { /* SanitizeReport */ } }
```
...
**params**
```json
{ "workspace_path": "…", "visibility": "public",
  "bump": "patch|minor|major|none" }
```
**progress** `{ "phase": "pack|sanitize|upload", "done": 0, "total": 0 }`
**result**
```json
{ "artifact_id": "…", "content_hash": "…", "status": "created|deduplicated",
  "worker_url": "…" }
```
...
**result**
```json
{ "pubkey_hex": "…64-hex…", "fingerprint_hex": "aa11bb22cc33",
