...
function buildEntries(s: SensorSnapshot): SensorEntry[] {
  ...
  if (s.frame.available) {
    ...
    entries.push({ label: 'Frame Time', value: fmt(s.frame.frame_time_ms, 1), unit: 'ms', category: 'frame' });
    entries.push({ label: 'Frame Avg', value: fmt(s.frame.frame_time_avg_ms, 1), unit: 'ms', category: 'frame' });
    entries.push({ label: 'Frame 1%', value: fmt(s.frame.frame_time_1percent_ms, 1), unit: 'ms', category: 'frame' });
    entries.push({ label: 'Frame 0.1%', value: fmt(s.frame.frame_time_01percent_ms, 1), unit: 'ms', category: 'frame' });
  } else {
  ...
  // CPU
  entries.push({ label: 'CPU Usage', value: fmt(s.cpu.total_usage_percent), unit: '%', category: 'cpu' });
  entries.push({ label: 'CPU Temp', value: fmt(s.cpu.package_temp_c), unit: '°C', category: 'cpu' });
  ...
  entries.push({ label: 'GPU Clock', value: fmt(s.gpu.core_clock_mhz), unit: 'MHz', category: 'gpu' });
  entries.push({ label: 'Mem Clock', value: fmt(s.gpu.mem_clock_mhz), unit: 'MHz', category: 'gpu' });
  entries.push({ label: 'VRAM', value: `${fmt(s.gpu.vram_used_mb)}/${fmt(s.gpu.vram_total_mb)}`, unit: 'MB', category: 'gpu' });
  entries.push({ label: 'GPU Power', value: fmt(s.gpu.power_draw_w), unit: 'W', category: 'gpu' });
  entries.push({ label: 'GPU Fan', value: fmt(s.gpu.fan_speed_percent), unit: '%', category: 'gpu' });
  ...
  entries.push({ label: 'RAM Total', value: s.ram.total_mb.toString(), unit: 'MB', category: 'ram' });
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
        {entries.map(entry => (
          <div key={entry.label} className="flex items-center justify-between py-0.5">
            ...
            <span className="text-xs font-mono font-medium" style={{ color: categoryColors[entry.category] }}>
              {entry.value}
