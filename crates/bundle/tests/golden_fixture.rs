...
//! ```text
//! WRITE_GOLDEN=1 cargo test -p bundle --test golden_fixture
//! ```
...
use bundle::{canonical_hash, FileEntry, Manifest, Tag};
use sha2::{Digest, Sha256};

fn sample_manifest() -> Manifest {
    // Intentionally identical to `crates/bundle/src/hash.rs::tests::sample()`
...
fn json_path() -> PathBuf {
    fixtures_dir().join("golden-manifest.json")
}
fn hex_path() -> PathBuf {
    fixtures_dir().join("golden-hash.hex")
}
...
fn golden_fixture_matches_canonical_output() {
    ...
    let json_on_disk = std::fs::read(json_path()).expect(
        "golden-manifest.json missing; run `WRITE_GOLDEN=1 cargo test -p bundle --test golden_fixture` to generate",
    );
    let hex_on_disk = std::fs::read_to_string(hex_path())
        .expect("golden-hash.hex missing; run with WRITE_GOLDEN=1 to generate");
