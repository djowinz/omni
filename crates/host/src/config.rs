    ...
    fn default() -> Self {
        Self {
            ...
            exclude: vec![
                ...
                "steamwebhelper.exe".to_string(),
                // Game launcher helpers / services (live inside game directories)
                "epicwebhelper.exe".to_string(),
                "epiconlineservicesuserhelper.exe".to_string(),
                "epiconlineserviceshost.exe".to_string(),
                "crashreportclient.exe".to_string(),
                "crashpad_handler.exe".to_string(),
                "unitycrashhandler64.exe".to_string(),
                "unitycrashhandler32.exe".to_string(),
                "ue4prereqsetup_x64.exe".to_string(),
                "dotnetinstaller.exe".to_string(),
                "vcredist_x64.exe".to_string(),
                "dxwebsetup.exe".to_string(),
                "bootstrappackagedgame-win64-shipping.exe".to_string(),
                "gameoverlayui.exe".to_string(),
                "gameoverlayui64.exe".to_string(),
                "steamoverlayvulkanlayer64.exe".to_string(),
                // Game-specific launchers that live in game dirs
                "marathonlauncher.exe".to_string(),
...
pub fn load_config(path: &Path) -> Config {
    if !path.exists() {
        ...
        if let Err(e) = save_config(path, &default) {
            tracing::warn!(path = %path.display(), error = %e, "Failed to save default config");
        }
        return default;
    }
