...
//!   1. Pack a signed bundle in-process via `omni_identity::pack_signed_bundle`.
...
async fn worker_to_host_to_workspace_roundtrip() {
    ...
    let manifest = Manifest {
        ...
        files: vec![
            FileEntry { path: "overlay.omni".into(), sha256: overlay_sha },
            FileEntry { path: "themes/theme.css".into(), sha256: theme_sha },
        ],
        resource_kinds: None,
    };
    ...
    let client = ShareClient::new(server.uri()).unwrap();
    ...
    assert!(theme_file.exists(), "themes/theme.css must exist after install");
    let theme_on_disk = std::fs::read(&theme_file).unwrap();
    assert_eq!(theme_on_disk, theme_bytes, "theme.css contents match fixture");

    let overlay_file = outcome.installed_path.join("overlay.omni");
    assert!(overlay_file.exists(), "overlay.omni must exist after install");
    let overlay_on_disk = std::fs::read(&overlay_file).unwrap();
    assert_eq!(overlay_on_disk, overlay_bytes, "overlay.omni contents match fixture");
    ...
    let entry = entries.get("e2e-theme").expect("entry keyed by display name");
